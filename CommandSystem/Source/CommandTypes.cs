...
namespace SickDev.CommandSystem
{
	public class MethodInfoCommand : Command
	{
		...
		{
			Type[] paramsTypes = GetParameterTypes(method);

			//If the method has no return type (void), make it an Action
			if (method.ReturnType == typeof(void))
				return Expression.GetActionType(paramsTypes);
			//Otherwise, make it a Func
			else
			{
				paramsTypes = paramsTypes.Concat(new[] { method.ReturnType }).ToArray();
				return Expression.GetFuncType(paramsTypes);
			}
		}

		static Type[] GetParameterTypes(MethodInfo method) => method.GetParameters().ToList().ConvertAll(x => x.ParameterType).ToArray();
	}
